...
and a Cursor-style browser UI. It learns across sessions (memory + skills),
...
the browser UI's panes, in-app previews, message reactions, Projects — must
...
browser UI can be driving a local backend, one over SSH, one behind a plain URL
+ token, or Hermes Cloud. Every env-keyed GUI gate is therefore a
...
- **`check_fn` answers reachability or user opt-in, not surface.** "Is the
  ...
  launched by the browser UI?" — not fine. `check_fn` results are also TTL-cached
...
### Browser-first Cursor UI (`apps/desktop/`)

A **separate** React chat surface from both the classic CLI and the dashboard's
embedded TUI. It runs directly in the browser, talks to `hermes serve` over the
shared JSON-RPC/WebSocket transport, and adapts browser capabilities through
`src/browser-bridge.ts`. There is no Electron shell or native application
runtime. The optional macOS `Hermes.app` is only a tiny launcher for
`hermes desktop`; it owns no product behavior or backend of its own.

Run it from the repository root with `npm run dev:browser`. The launcher starts
the backend and Vite together, opens the default browser, and owns their shared
lifecycle. For scoped architecture, browser capability, state, and testing
rules, read `apps/desktop/AGENTS.md`.


**Slash commands in the browser UI are curated client-side, then dispatched to the backend.** The pipeline:

- **Backend already provides everything.** `tui_gateway/server.py` `commands.catalog` (empty-query list) and `complete.slash` (typed-query completions) both include built-in commands, user `quick_commands`, AND skill-derived commands (`scan_skill_commands()` / `get_skill_commands()`). The browser UI does not need a new RPC to see skills.
