...
# what protects stalled dependency downloads on macOS, where `timeout` is
# usually absent, turning an indefinite hang into a non-zero exit.
#
...
run_with_timeout() {
    ...
    # Normalize to a bare integer; fall back to the installer default if a caller
    ...
    # subtree (npm and package installers may spawn child processes).
    set -m
    ( "$@" ) &
    local cmd_pid=$!
    set +m
...
# checkout must cd into it explicitly. Without this, install_deps/setup_path can
# resolve `.` / the venv against the caller's working directory.
require_install_dir() {
...
# Structured installer stage protocol, shared with the Windows installer.
run_stage_body() {
    local stage="$1"
...
run_stage_protocol() {
    ...
    # before emitting the frame and an installer driver would see "no result
    # frame" instead of a clean {ok:false} contract response.
    set +e
