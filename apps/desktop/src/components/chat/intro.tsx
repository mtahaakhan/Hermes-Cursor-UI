...
const SUGGESTIONS = [
  { title: 'Start with a plan', detail: 'Align on implementation before writing code', prompt: 'Start with a plan: ' },
  { title: 'Debug an issue', detail: 'Find root causes and fix tricky bugs', prompt: 'Debug this issue: ' },
  { title: 'Create a dashboard', detail: 'Visualize information in a custom interface', prompt: 'Create a dashboard for ' }
] as const
...
export function Intro({ personality, seed }: IntroProps) {
  ...
  return (
    <div
      className="pointer-events-none flex w-full min-w-0 flex-col items-center justify-end px-0.5 pb-2 pt-6 text-muted-foreground sm:px-6 lg:px-8"
      data-slot="aui_intro"
    >
      <div className="pointer-events-auto w-full min-w-0 max-w-[46rem]">
        <h1 className="sr-only">What are you building?</h1>
        <div className="cursor-intro-suggestions">
          {SUGGESTIONS.map(item => (
            <button
              className="cursor-intro-suggestion"
              key={item.title}
              onClick={() => requestComposerInsert(item.prompt, { mode: 'inline', target: 'main' })}
              type="button"
            >
              <span className="cursor-intro-suggestion-icon">
                <Codicon name="chevron-right" size="0.75rem" />
              </span>
              <span className="cursor-intro-suggestion-copy">
