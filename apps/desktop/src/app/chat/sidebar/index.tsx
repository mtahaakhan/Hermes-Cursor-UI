...
const SIDEBAR_NAV: SidebarNavItem[] = [
  {
    id: 'new-session',
    label: 'New Chat',
    icon: props => <Codicon name="filter" {...props} />,
    action: 'new-session',
...
export function ChatSidebar({
  ...
  return (
    <Sidebar
      ...
      collapsible="none"
    >
      <SidebarContent className="cursor-sidebar-shell gap-0 overflow-hidden bg-transparent">
        <SidebarGroup className="cursor-sidebar-nav shrink-0 p-0">
          <SidebarGroupContent>
            <SidebarMenu className="gap-0.5">
              {[...SIDEBAR_NAV, ...contributedNav].map(item => {
                const isSearch = item.id === 'search'
                const isInteractive = Boolean(item.action) || Boolean(item.route) || isSearch

                const active =
                  (item.id === 'new-session' && currentView === 'chat' && !selectedSessionId) ||
                  (item.id === 'skills' && currentView === 'skills') ||
                  (item.id === 'cron' && currentView === 'cron') ||
                  (isSearch && (searchOpen || Boolean(trimmedQuery))) ||
                ...
                const button = (
                  <SidebarMenuButton
                    ...
                    className={cn(
                      ...
                      // thread.tsx.
                      'cursor-nav-item flex h-8 w-full justify-start gap-2.5 rounded-[8px] border border-transparent px-2 text-left text-[0.8125rem] font-medium text-(--ui-text-secondary) transition-colors duration-100 ease-out [-webkit-app-region:no-drag] hover:bg-(--ui-control-hover-background) hover:text-foreground hover:transition-none',
                      isNewSession && 'cursor-new-chat',
                      active && 'cursor-nav-active text-foreground',
                      !isInteractive &&
        ...
        <div className="cursor-chats-column">
          {showSessionSections && (searchOpen || trimmedQuery) && (
            <div className="cursor-explorer-search">
              <SearchField
                aria-label={s.searchAria}
                containerClassName="cursor-search-field"
        ...
        {showSessionSections && (
          <div className={cn('flex min-h-0 flex-1 flex-col pb-1.75', SCROLL_Y, SCROLL_GUTTER)}>
            ...
            {!trimmedQuery && !worktreeGroupingActive && cronJobs.length > 0 && (
              <div className="cursor-sidebar-cron">
        ...
        <div className="cursor-profile-footer">
          <ProfileRail />
        </div>
      </SidebarContent>
      <ProjectDialog />
