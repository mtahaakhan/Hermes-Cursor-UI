...
Two surfaces can start an update of the same install tree:

* ``hermes update`` from a terminal,
* the dashboard's Update button (``POST /api/hermes/update`` →
  ``_spawn_hermes_action(["update"])``, detached).

The marker prevents two updaters from mutating the same checkout concurrently,
rewriting source under a live interpreter and leaving the tree half-updated.

This module provides one lock for all update entrypoints:
...
than :data:`UPDATE_MARKER_MAX_AGE_SECONDS`, so a
...
# A full update (git pull + dependency sync) can take several minutes.
UPDATE_MARKER_MAX_AGE_SECONDS = 20 * 60

MARKER_NAME = ".hermes-update-in-progress"

# Backward-compatible handoff for older external update orchestrators that
# spawn `hermes update` while holding the marker themselves.
...
# _cmd_update_impl exits 2 for concurrent update refusal.
