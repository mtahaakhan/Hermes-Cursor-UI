...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    if (
        ...
        # --workspace ui-tui keeps this install scoped to the terminal UI.
        # See #38772.
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    Uses a SHA-256 content hash of the web source tree, not mtime comparison.
    ``git checkout`` / ``git pull`` / ``hermes update``
...
def _nixos_build_env() -> dict[str, str] | None:
    """Return a Python build environment for Node native addons on NixOS."""
    import re
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    builds (web UI / TUI / browser UI workspaces), and those builds need the dev
    toolchain (``tsc`` and ``vite`` — both
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # is never resolved here. Without --workspace the root package.json's
    # apps/* glob would pull in every app on each web build. See #38772.
...
def cmd_gui(args: argparse.Namespace):
    """Launch the browser-first Hermes Cursor UI."""
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
    if not (desktop_dir / "package.json").exists():
        print(f"Browser UI source not found at: {desktop_dir}")
        sys.exit(1)

    if getattr(args, "install_app", False):
        from hermes_cli.browser_app import install_default_macos_browser_app

        try:
            installed = install_default_macos_browser_app(project_root=PROJECT_ROOT)
        except Exception as exc:
            print(f"Could not install Hermes browser launcher: {exc}")
            sys.exit(1)
        print(f"✓ Installed browser launcher → {installed.app_path}")
    ...
    if requested_cwd:
        cwd = str(Path(requested_cwd).expanduser().resolve())
...
def cmd_update(args):
    ...
    # Cross-process mutual exclusion. The dashboard's Update button can spawn
    # this same command detached; two updates running together can rewrite
    # source under a live interpreter and strand the tree half-updated.
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    # Remote clients write tokens under $HOME/.hermes/desktop-ssh, independent
    # of HERMES_HOME and the active profile. Anchor validation to that same
    # OS-home path, NOT to
...
def main():
    ...
    #
    # The canonical name is "desktop" for command compatibility; "gui" is an
    # alias. Both launch the Cursor-style interface in the system browser.
    # =========================================================================
