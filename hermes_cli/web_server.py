...
async def fs_write_text(payload: FsWriteText):
    ...
    Applies the same hardening as local filesystem writes: the path is
...
#
# Local clients can run git on the user's machine; over a
...
def _ws_host_origin_reason(ws: "WebSocket") -> Optional[str]:
    ...
    if parsed.scheme not in {"http", "https"}:
        # Non-web origin (for example file:// or app://). The
...
async def shell_ws(ws: WebSocket) -> None:
    """Run an interactive system shell for the browser UI terminal pane."""
    auth_reason, _cred = _ws_auth_reason(ws)
    if auth_reason is not None:
        await ws.close(code=4401, reason=_ws_close_reason(f"auth: {auth_reason}"))
        return

    request_reason = _ws_host_origin_reason(ws) or _ws_client_reason(ws)
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text("\r\nTerminal unavailable: no PTY implementation is installed.\r\n")
        await ws.close(code=1011)
        return

    raw_cwd = (ws.query_params.get("cwd") or "").strip()
    cwd_path = Path(raw_cwd).expanduser() if raw_cwd else Path.cwd()
    ...
    try:
        bridge = PtyBridge.spawn(argv, cwd=str(cwd_path), cols=cols, rows=rows)
    except (PtyUnavailableError, FileNotFoundError, OSError) as exc:
        await ws.send_text(f"\r\nTerminal failed to start: {exc}\r\n")
        await ws.close(code=1011)
        return
...
def _start_launcher_watchdog() -> None:
    """Stop a browser-mode backend when the launcher process disappears."""
    raw_pid = (os.environ.get("HERMES_LAUNCHER_PID") or "").strip()
    if not raw_pid:
        return
    try:
    ...
    def _watch() -> None:
        while _launcher_alive():
            time.sleep(1)
        os._exit(0)

    threading.Thread(target=_watch, daemon=True, name="browser-launcher-watchdog").start()
...
def start_server(
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            # Machine-readable port-discovery sentinel for launchers.
