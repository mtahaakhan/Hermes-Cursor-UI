...
def _update_node_dependencies() -> list[str]:
    ...
    # Install the root and required workspaces deliberately so an update does
    # not resolve unrelated frontend packages.
    print("→ Updating Node.js dependencies...")
...
def _log_only_write(text: str) -> None:
    ...
    subprocess output (npm installs, Vite builds, the cua-driver
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        # avoid forcing reinstalls. But on a managed install (identified by
        # `.install_method`, or by an interrupted update marker), the venv is
        # the install — its absence means a repair was interrupted after the
        # old venv moved aside.
        managed_markers = (
            _m().PROJECT_ROOT / ".install_method",
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # The Cursor UI is browser-first and has no native package to rebuild.
        # Its Vite development server compiles source on demand.
        print()
        print("✓ Code updated!")
